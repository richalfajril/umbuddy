...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  if (!userId) {
    return apiErrorResponse('UNAUTHORIZED', 'Kamu perlu masuk dulu untuk menyimpan jawaban.', 401)
  }
  ...
  if (validation.data === null) {
    return apiErrorResponse('VALIDATION_ERROR', 'Jawaban latihan belum valid.', 400, validation.errors)
  }

  const answers = validatePracticeAnswers(validation.data.answers)
  if (!answers.ok) {
    return apiErrorResponse('VALIDATION_ERROR', 'Jawaban latihan belum valid.', 400, answers.errors)
  }
